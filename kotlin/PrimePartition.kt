...
fun printResults(primes: List<Long>, counts: Map<Long, Int>) {
 ...
 */
fun <T> binaryPartitions(list: List<T>): Sequence<Pair<List<T>, List<T>>> = sequence {
 ...
 */
fun <T> combinations(k: Int, list: List<T>): Sequence<List<T>> = sequence {
    if (k == 0) {
        yield(emptyList())
        return@sequence
    }
 ...
 */
fun exponentCombinations(n: Int, maxExp: Int): Sequence<List<Int>> = sequence {
 ...
 */
fun productPow(list: List<Long>, exps: List<Int>): Long {
 ...
 * Integer square root
 */
fun isqrt(n: Long): Long = sqrt(n.toDouble()).toLong()

/**
 * Helper: replicateM (Cartesian product of n copies of a list)
 */
fun <T> replicateM(n: Int, list: List<T>): List<List<T>> {
 ...
 */
fun <T> List<T>.splitPair(n: Int): Pair<List<T>, List<T>> {
